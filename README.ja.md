...
<p align="center">
  Claude Code、Codex、Grok、Grok Bot、Cursorの使用率とリセット時刻を、macOSのメニューバーとウィジェットで確認できる無料のオープンソースアプリです。
</p>
...
- **ウィジェット** — デスクトップに使用率とリセット時刻を表示。小サイズは1エージェント、中サイズは4行まで（枠を複数持つエージェントは複数行を使うため、概ね3エージェント）、大サイズは5エージェントまでで、各バーの下に残りとリセットまでの時間も表示します。表示するエージェントはカード順で決まり、「ウィジェットを編集」から各サイズで表示できる数まで選ぶこともできます
...
| **Codex** | Codex公式クライアント本体（`codex app-server`）。無い場合は`~/.codex/sessions/**/rollout-*.jsonl` | 利用可能なprimary／secondary枠、リセット時刻、プラン |
...
Codexは公式クライアントの`codex app-server`を起動し、`/status`と同じ値を尋ねます。`codex`はコマンドラインツールの一般的な配置先と`ChatGPT.app`の中から探すため、CLIと公式アプリのどちらか一方があれば取得できます。`~/.codex`へは何も書き込まず、モデルも動きません。クライアントが無い場合はセッションログを読みますが、こちらはターミナル版Codexを使ったときしか更新されません。Grokは従来どおりローカルログを読む受動的な方式で、Grokを利用してログが更新されるまで表示値も変わりません。
...
Gemini CLIは、現在のところ必要な使用枠情報をローカルで安全かつ安定して取得できないため未対応です。取得方法を確認できたツールから順次追加する予定です。
...
6. 必要なら、デスクトップの「ウィジェットを編集」からAI Usageを追加します。カード順に先頭から、小サイズは1エージェント、中サイズは4行、大サイズは5エージェントまでを表示します。変えるには設定でカードを並べ替えるか、ウィジェットを右クリックして「ウィジェットを編集」から選びます。
...
- Codex：CLIか公式アプリのいずれかが入っていれば自動で最新値を取得します。どちらも無い場合はCodexで通常の応答を完了してローカルログを更新
- Grok：Grokで通常の応答を完了してローカルログを更新
- 共通：`Stale`バッジまたはヘッダーのヘルプから、エージェント別の対処方法を確認

## プライバシーと通信

- メニューバー本体は、GrokのローカルログとClaude CodeのKeychain認証情報を読み、Codex公式クライアントを起動するため非サンドボックスです。
- WidgetKit拡張はサンドボックス化され、App Group経由で必要最小限の表示用スナップショットだけを受け取ります。
- **AI Usage自身が外部通信するのは、Claude Code・Grok Bot・Cursorの取得時だけ**です。Claude Codeの認証情報はAnthropicのusage APIとOAuth token endpointにだけ送信し、Grok BotとCursorは設定で保存したセッションをcursor.comのダッシュボードAPIにだけ送信します。
- Codexへの問い合わせは公式クライアントが自身の認証情報で行います。AI Usageは通信せず、Codexの認証情報も読まず、`~/.codex`へ書き込みもしません。モデルは動かないためトークンも消費しません。Grokの収集では通信もトークン消費も発生しません。
...
- CodexはCLIも公式アプリも無い場合、Grokは常に、ローカルログが更新されるまで値が変わりません。
